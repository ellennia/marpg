...
Each scene has a list of adjacent scenes. These are scenes which in the game are visible and directly accesssible to be moved into when the player-character is in that scene. For example, if *s1* is adjacent to *s2*, and *s2* is adjacent to *s3*, the user can move from *s1* to *s2*, though not to *s3* directly. However, a user in *s2* can move to *s3*, and one in *s3* to *s2*.
